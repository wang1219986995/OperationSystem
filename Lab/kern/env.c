...
region_alloc(struct Env *e, void *va, size_t len)
{
    ...
    for(i = ROUNDDOWN((uint32_t)va, PGSIZE); i < ROUNDUP((uint32_t)va + len, PGSIZE); i += PGSIZE)
    {
        ...
        if(info == NULL)    panic("region_alloc:can't alloc memory.\n");
        page_insert(e->env_pgdir, info, (void*)i, PTE_U | PTE_W | PTE_P);
    }
}
...
load_icode(struct Env *e, uint8_t *binary)
{
    ...
    if(elf->e_magic != ELF_MAGIC)
        panic("locd_icode: elf is not correct.\n");

    if(elf->e_entry == 0)    
        panic("load_icode: elf entry is invalid.\n");
	...
	for (; ph < eph; ph++)
    {
        ...
        if(ph->p_filesz > ph->p_memsz)
            panic("load_icode: ph filesz > memsz\n");
