...
boot_alloc(uint32_t n)
{
    ...
    else if(n > 0)
    {
        ...
        // Judge whether out of memory
        if((size_t)nextfree > (size_t)0xffffffff - ROUNDUP((size_t)n, PGSIZE))
        {
            panic("boot_alloc has out of memory!");
            return NULL;
        }
...
page_init(void)
{
    ...
    for(i = 0; i < npages; i++)
    {
        if(i == 0 || (i >= (IOPHYSMEM / PGSIZE) && i < ((PADDR(boot_alloc(0))) / PGSIZE)))
        {
		    pages[i].pp_ref = 1;
...
page_free(struct PageInfo *pp)
{
    ...
    {
        panic("Should't free the page:0x%x \n", pp);
    }
...
page_remove(pde_t *pgdir, void *va)
{
    ...
    struct PageInfo* info = page_lookup(pgdir, va, 0);
    if(info != NULL)
    {
        tlb_invalidate(pgdir, va);
        page_decref(info);
